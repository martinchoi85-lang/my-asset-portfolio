...
### Phase 2: 대시보드 뷰어 및 사용자 맞춤 UI 개편 (UI/UX Revamp) [✅ 완료]
**해결됨:** 수동 자산 관리를 전용 메뉴로 분리(Two-Track UI), 및 "정적 자산 평가액 갱신" 일괄 업데이트 기능 도입으로 3-Depth 탭 및 계좌 전환 피로도 최소화 완료. 비즈니스 폼(만기/해지/이자수령) 및 납입원금 직접 수정(데이터 보정) 기능 적용.
...
### Phase 3.1: 시스템 관리 UI 확장 및 계좌/템플릿 에디터 도입
**다음 과제:** HTS 템플릿의 유연한 관리와 안전한 계좌 생성을 위한 전용 UI 구축.
- **카테고리 메뉴 구성 개편:**
  - `🛠️ 시스템 관리` 메뉴 하위에 [HTS 템플릿 관리], [Asset Editor (V1)], [계좌 관리] 라디오 버튼 3종을 신설하여 화면 공간을 효율화하고 접근성을 높입니다.
- **계좌 관리 (Account Editor) 신설:**
  - 기존 데이터 안전성을 위해 수정/삭제는 배제한 '추가 전용' 폼을 구현합니다.
  - `[증권사]_[계좌닉네임]_[유저명]` 형태의 자동 네이밍 규칙을 강제하여 데이터 일관성을 확보합니다.
- **HTS 템플릿 관리 (Profile Editor) 신설:**
  - `ImportProfile` 설정을 사용자가 UI 상에서 직접 조회하거나 신규 등록(Column/TradeType Mapping)할 수 있는 에디터 환경을 제공합니다.

### Phase 4: 시스템 기반 안정화 (Performance & Session)
**현재 병목:** 다수 거래 입력시 딜레이 극심, F5(새로고침) 시 세션 초기화.
- **상태 관리 및 세션(Session) 유지 확보:**
  - PC 접속 시에는 Streamlit으로, 모바일 접속 시에는 별도의 React 앱으로 접속하여 환경의 한계를 줄이고, 토큰(혹은 로컬/쿠키 스토리지) 기반 사용자 세션 유지 로직 도입.
- **퍼포먼스 개선:**
  - 분석 화면에서 개별 종목의 차트를 그릴 때도 해당 자산의 데이터만 가져오면 되는데 streamlit의 특성 상 페이지 전체를 리프레시 하기 때문에 속도도 느리고, 사용성이 저하되므로 한 페이지에 그리는 컴포넌트들은 가능하면 동일 데이터를 사용하도록 그룹화하여 한 번의 리프레시로 처리되도록 개선.
