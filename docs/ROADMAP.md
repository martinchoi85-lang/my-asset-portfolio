...
1. **스냅샷 생성(Daily Snapshot) 로직 고도화 및 최적화**
   - 과거 스냅샷 생성 시 `current_price` 참조로 인한 평가 왜곡 문제 개선 (Fallback 로직 변경)
   - 트랜잭션 누적 계산 시 전체 히스토리 로딩 대신 이전 스냅샷 상태를 활용하는 방식으로 성능 최적화
   - 스케줄러 도입 (GitHub Actions or Cron)
2. **다중 통화(Multi-currency) 아키텍처 재설계**
   - 현재 대시보드 쪽(`fx_utils`)에 치중된 환율 변환 로직을 백엔드 계산 모듈(`portfolio_calculator`)로 이관
   - Snapshot에서 Base Currency 형태의 일관된 데이터 산출
