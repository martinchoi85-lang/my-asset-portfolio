...
1. **[x] 스냅샷 생성(Daily Snapshot) 로직 고도화 및 최적화**
   - [x] 과거 스냅샷 생성 시 `current_price` 참조로 인한 평가 왜곡 문제 개선 (Fallback 로직 변경 완성)
   - [x] 트랜잭션 누적 계산 시 전체 히스토리 로딩 대신 이전 스냅샷 상태를 활용하는 방식으로 성능 최적화 (Incremental Build 적용 완료)
   - [ ] 스케줄러 도입 (GitHub Actions or Cron)
2. **[x] 다중 통화(Multi-currency) 아키텍처 재설계 및 환율 최적화**
   - [x] 단일 환율 일괄 변환으로 인한 과거 스냅샷 시계열(투자원금 등) 차트 왜곡 현상 개선 (과거 환율 데이터 수집 기능 추가)
   - [x] 기간 내 성과 계산 및 실현손익 산출 시 해당 거래일 당시의 환율을 매핑하여 반영하는 로직으로 재설계 (`portfolio_service.py`)
   - [x] 기초 백엔드 성능 및 대시보드 변환부를 과거 환율 데이터프레임과 병합하는 구조로 최적화
3. **데이터 무결성 검증**: 마이너스 잔고 방지, 중복 거래 방지 로직 강화
4. **테스트 코드 작성**: 주요 계산 로직(수익률, 평단가) 단위 테스트
5. **데드 코드 파괴 및 패키지 구조 클린업**
   - 분산된 유틸리티 파일 통합 및 레거시 주석 코드 제거