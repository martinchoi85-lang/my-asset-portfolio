...
│       ├── daily_snapshot_generator.py
│       ├── data_contracts.py
│       ├── fx_service.py
│       ├── holding_period_service.py
│       ├── importer
│       │   ├── base.py
│       │   ├── engine.py
│       │   └── profiles.py
│       ├── krx_price_fetcher.py
│       ├── lookthrough_service.py
│       ├── manual_cost_basis_service.py
...
## 2. Function List by File

### `src\asset_portfolio\backend\__init__.py`
- (No functions defined)

### `src\asset_portfolio\backend\infra\__init__.py`
- (No functions defined)

### `src\asset_portfolio\backend\infra\query.py`
- `def get_user_by_password()`: 비밀번호로 사용자를 조회합니다.
- `def get_accounts()`: 특정 사용자의 모든 계좌 정보를 불러옵니다.
- `def create_account()`: 새 계좌를 생성합니다.
- `def get_import_profiles()`: 사용자의 HTS 통계 설정 리스트를 불러옵니다.
- `def upsert_import_profile()`: HTS 통계 설정을 갱신 또는 추가합니다.
- `def _as_date_str()`
...
### `src\asset_portfolio\backend\infra\supabase_client.py`
- `def get_supabase_client()`

### `src\asset_portfolio\backend\services\asset_alias_service.py`
- `class AssetAliasService`
  - `def get_alias_map()`: 사용자의 모든 자산 별칭(Alias)을 조회하여 {alias_name: asset_id} 딕셔너리로 반환합니다.
  - `def add_alias()`: 새로운 자산 별칭을 등록하거나 기존 별칭을 업데이트합니다.
  - `def delete_alias()`: 자산 별칭을 삭제합니다.

### `src\asset_portfolio\backend\services\asset_handler.py`
- `class AssetHandler`: 자산의 유형별(수동/자동 등) 트랜잭션, 스냅샷 리빌드 등 공통 인터페이스
  - `def __init__()`
  - `def is_manual()`
  - `def rebuild_snapshots()`: 이 자산에 대한 스냅샷 리빌드 정책 실행
  - `def calculate_snapshots()`: 이 자산에 대한 스냅샷 데이터를 계산
- `class AutoAssetHandler`: 일반적인 시장/자동 평가 자산 핸들러
  - `def is_manual()`
  - `def rebuild_snapshots()`
  - `def calculate_snapshots()`
- `class ManualAssetHandler`: 수동 기입 자산(예적금, 부동산 등) 핸들러
  - `def is_manual()`
  - `def rebuild_snapshots()`: 수동 자산은 사용자 입력 값을 우선하되, 
  - `def calculate_snapshots()`: 수동 자산용 스냅샷 계산기:
  - `def calculate_withdrawal_cost_delta()`: 수동 자산의 인출 시 비례 차감될 원금(delta)을 계산한다.
- `class AssetManager`: 자산 ID의 메타데이터를 기반으로 적절한 AssetHandler를 반환하는 팩토리
  - `def get_handler()`

### `src\asset_portfolio\backend\services\asset_service.py`
- `class AssetService`
...
### `src\asset_portfolio\backend\services\benchmark_service.py`
...
### `src\asset_portfolio\backend\services\daily_snapshot_generator.py`
- `def generate_daily_snapshots()`: 특정 account에 대해

### `src\asset_portfolio\backend\services\data_contracts.py`
- `def _ensure_columns()`
...
### `src\asset_portfolio\backend\services\fx_service.py`
- `class FxRate`: ✅ FX 결과를 명확히 남기기 위한 구조
- `class FxService`: ✅ USDKRW 환율을 '대시보드 합산용'으로 제공
...
### `src\asset_portfolio\backend\services\holding_period_service.py`
- `def _to_date()`
- `def calculate_holding_periods()`: TWR/FIFO 기반으로 자산별 보유 기간(Holding Period)을 계산합니다.

### `src\asset_portfolio\backend\services\krx_price_fetcher.py`
- `class KRXPriceResult`: ✅ KRX 가격 조회 결과
- `class KRXPriceFetcher`: ✅ KRX(한국거래소) 자동 가격 수집기
  - `def _normalize_code()`
...
### `src\asset_portfolio\backend\services\lookthrough_service.py`
- `def apply_lookthrough_to_grouping_df()`: 자산군 집계 DataFrame에서 Look-through 대상 자산(펀드, TDF 등)을 

### `src\asset_portfolio\backend\services\manual_cost_basis_service.py`
...
- `def _calculate_auto_snapshots_for_asset()`: 특정 자동(Auto) 자산에 대해 일별 snapshot 데이터를 계산한다.
...
### `src\asset_portfolio\backend\services\portfolio_weight_service.py`
...
### `src\asset_portfolio\backend\services\price_updater_service.py`
- `class PriceUpdateResult`: ✅ 가격 업데이트 결과를 UI에 그대로 표시하기 위한 데이터 구조
- `class PriceUpdaterService`: ✅ 목적:
...
### `src\asset_portfolio\backend\services\transaction_service.py`
...
### `src\asset_portfolio\backend\services\importer\base.py`
- `class ImportProfile`: 특정 HTS 화면의 데이터 포맷 및 파싱 규칙을 정의하는 클래스입니다.
- `class ParsedTransaction`: 임포트 엔진에 의해 표준화된 단일 거래 데이터입니다.

### `src\asset_portfolio\backend\services\importer\engine.py`
- `class TransactionParser`
  - `def __init__()`
  - `def parse()`: 주어진 DataFrame을 프로파일에 따라 파싱하여 표준 거래 리스트로 반환합니다.
  - `def _parse_row()`
  - `def _clean_numeric()`
  - `def merge_2_row_hts()`: 2-row 구조의 HTS 데이터를 1-row로 병합하는 유틸리티 함수입니다.

### `src\asset_portfolio\backend\services\importer\profiles.py`
- `def kiwoom_2row_preprocess()`: 키움증권 [0365] 화면 등 2줄로 표시되는 데이터를 1줄로 병합합니다.
- `def mirae_domestic_preprocess()`: 미래에셋 국내주식 매매내역 전처리 (필수 필드 보강 버전)
- `def get_profile_by_name()`

### `src\asset_portfolio\dashboard\__init__.py`
- (No functions defined)

### `src\asset_portfolio\dashboard\account_editor.py`
- `def render_account_editor()`

### `src\asset_portfolio\dashboard\ai_report.py`
- (No functions defined)
...
### `src\asset_portfolio\dashboard\asset_editor.py`
...
### `src\asset_portfolio\dashboard\price_updater.py`
- `def _render_auto_updater()`
- `def _render_manual_updater()`
- `def render_price_updater()`

### `src\asset_portfolio\dashboard\profile_editor.py`
- `def _safe_json_dumps()`
- `def render_profile_editor()`

### `src\asset_portfolio\dashboard\recurring_order_editor.py`
- `def _load_accounts_df()`
- `def _load_assets_df()`
- `def render_recurring_order_editor()`

### `src\asset_portfolio\dashboard\render.py`
...
### `src\asset_portfolio\dashboard\static_asset_action.py`
- `def render_static_asset_actions()`

### `src\asset_portfolio\dashboard\transaction_editor.py`
- `def _load_accounts_df()`
- `def _load_assets_df()`
...
### `src\asset_portfolio\dashboard\transaction_importer.py`
...
### `src\asset_portfolio\mobile\__init__.py`
- (No functions defined)

### `src\asset_portfolio\mobile\app.py`
...
### `src\asset_portfolio\mobile\data.py`
